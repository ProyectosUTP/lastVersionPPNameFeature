var uploadEstimates = (function ( $, rateCard, toastr, Papa ) {
    var $fileInput;
    var featureFlagPricePeriodEnabledForCsvUploadFile = false;

    var showConfirmationDialog = (message) => {
        return new Promise((resolve, reject) => {
            toastr.dialog
                .warning(message)
                .addButton('Cancel', reject, true)
                .addButton('Ok', resolve);
        });
    }
    ...
    var validateEstimateUpload = function (estimates) {
        rateCard.ajax({
            ...
            type: 'POST',
                ...
                if (validationData.IsValid && validationData.ValidationMessages.hasOwnProperty("UploadSuccess")) {
                    ...
                    showConfirmationDialog(validationMessage).then(() => {
    ...
    var convertToDto = function (nonDemoColumns, headerData, row) {
        ...
        if (featureFlagPricePeriodEnabledForCsvUploadFile) {
            rowEstimate.PricePeriod = { YearNumber: row[nonDemoColumns.Year], QuarterNumber: row[nonDemoColumns.Quarter], Name: row[nonDemoColumns.PPName] };
        } else {
            rowEstimate.Quarter = { QuarterNumber: row[nonDemoColumns.Quarter], Year: row[nonDemoColumns.Year] };
        }
    ...
    var validateFileContentWithSpecificValidationMessages = function (nonDemoColumns, rows, headerData) {
        ...
        Papa.parse(file, {
            ...
            complete: function (result) {
                ...
                if (errors.length > 0) {
                    ...
                    if (featureFlagPricePeriodEnabledForCsvUploadFile) {
                        nonDemoColumns.PPName = "PRICE PERIOD NAME";
                        validateFileContentResult = validateFileContentWithSpecificValidationMessages(nonDemoColumns, dataArray, headerData);
                    } else {
                        validateFileContentResult = validateFileContents(nonDemoColumns, dataArray, headerData);
                    }

                    //Validate file content(client side) before converting to server side estimate Dto (SaveEstimateDto).
                    if (validateFileContentResult) {
    ...
    const initialize = function (options) {
        ...
        var advisorSettings = window.advisorSettings || {};
        if (advisorSettings.featureFlagPricePeriodEnabledForCsvUploadFile !== null && advisorSettings.featureFlagPricePeriodEnabledForCsvUploadFile !== undefined) {
            if (advisorSettings.featureFlagPricePeriodEnabledForCsvUploadFile.toString().toUpperCase() === 'TRUE')
    ...
    const testingOnly = {
        $fileInput,
        featureFlagPricePeriodEnabledForCsvUploadFile,
        showConfirmationDialog,
        validateEstimateUpload,
        uploadEstimate,
        convertToDto,
        validateFileContents,
        validateFileContentWithSpecificValidationMessages,
        handleChange
    };

    testingOnly.override = {
        $fileInput: (mock) => $fileInput = mock,
        featureFlagPricePeriodEnabledForCsvUploadFile: (mock) => featureFlagPricePeriodEnabledForCsvUploadFile = mock,
        showConfirmationDialog: (mock) => showConfirmationDialog = mock,
        ...
        validateFileContentWithSpecificValidationMessages: (mock) => validateFileContentWithSpecificValidationMessages = mock,
